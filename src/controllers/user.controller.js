...
const getUserWatchHistory = asyncHandler(async function(req, res) {  
    const user = await User.aggregate([
        {
            $match: {   // here in other code there is no need of using String fucn but we're getting error so
                _id: new mongoose.Types.ObjectId(String(req.user._id))  // that's why we're using String function to 
            }                                                           // check first where the user._id is String or not
        },
        {
            $lookup: {
                from: "videos",
